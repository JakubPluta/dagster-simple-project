...
mflix_mongodb_connection = dlt_mongodb.mongodb(database=DATABASE_NAME).with_resources(
    *COLLECTIONS
)
...
def dlt_asset_factory(context: AssetExecutionContext, dlt: DagsterDltResource):
    yield from dlt.run(context=context, write_disposition=WRIITE_DISPOSITION)
